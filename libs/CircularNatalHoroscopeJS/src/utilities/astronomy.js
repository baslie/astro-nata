...
export const getObliquityEcliptic = ({ julianDate = 2451545.0 } = {}) => {
...
export const getAscendant = ({ latitude = 0.00, obliquityEcliptic = 23.4367, localSiderealTime = 0.00 } = {}) => {
  // Meeus "Astronomical Algorithms" — using atan2 for correct quadrant
  latitude = parseFloat(latitude);
  obliquityEcliptic = parseFloat(obliquityEcliptic);
  localSiderealTime = parseFloat(localSiderealTime);

  const y = cosFromDegrees(localSiderealTime);
  const x = -(sinFromDegrees(obliquityEcliptic) * tanFromDegrees(latitude)
            + cosFromDegrees(obliquityEcliptic) * sinFromDegrees(localSiderealTime));

  let ascendant = radiansToDegrees(Math.atan2(y, x));
